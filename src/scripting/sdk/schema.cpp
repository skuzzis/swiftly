...
void SchemaCallback(PluginObject plugin, EContext* ctx) {
    ...
    ADD_FUNCTION("GenerateSDKFactory", [](FunctionContext* context) -> void {
        EValue ent = MakeSDKClassInstance(context->GetArgument<std::string>(0), nullptr, context->GetPluginContext());
        context->SetReturn(ent);
    });

    ADD_CLASS_FUNCTION("SDKClass", ctx->GetKind() == ContextKinds::Lua ? "__call" : "call", [](FunctionContext* context, ClassData* data) -> void {
    ...
    auto sdkfields = g_sdk.GetProcessedFieldNames();
    for(auto it = sdkfields.begin(); it != sdkfields.end(); ++it)
        ADD_CLASS_MEMBER("SDKClass", *it, DummyGetSetSDK, DummyGetSetSDK);
